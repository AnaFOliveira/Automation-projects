...
def tag_frequency(text):
    ...
    trace = {"type": "bar",
             "x": new_freq_list, "y": word_list,
             "marker": {"color": "rgb(222, 113, 88)"},
             "orientation": "h"
             }

    layout = go.Layout({
        'xaxis': {'title': "Relative Frequency", 'showgrid': False, 'showticklabels': False, 'zeroline': False},
...
def hours_plot_original(df):
    ...
    trace = ff.create_distplot(columns, group_labels, bin_size=.25)
    new_fig = go.Figure(trace)
    new_fig.update_layout(title='Most common hours of Strokes (or of the registration)',
                          xaxis={'title': 'Hours'},
...
def initialize_app(db):
    ...
    column = db["SeriesDescription"]
    descriptions = to_text(column)
    ...
    app.layout = html.Div(children=[
        ...
        html.H3(children='Methinks vision is to provide universal and timely medical assistance to enable life-saving '
